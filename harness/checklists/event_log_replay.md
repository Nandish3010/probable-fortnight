...
- [ ] Replay of a golden run renders the same panel states as the live run (snapshot diff)
- [x] `GET /events/{run_id}` and the SSE stream emit the same sequence
- [x] Replay timing: original or 4x

## Reviewer-verified
- [ ] Trace is readable to a judge (`invocation_id` visible, one line per tool call, revision visible)
