...
def test_required_routes_exist():
    ...
    for p in ("/health", "/approve", "/rerun", "/events/{run_id}", "/events/{run_id}/stream", "/chat", "/reset", "/capture", "/gaps", "/plays", "/plays/{play_id}", "/outcomes", "/execution", "/policy", "/trends", "/trends/recompute", "/feedback", "/feedback/form", "/feedback/summary", "/feedback/{response_id}"):
        assert p in paths, p
